...
from apps.recommendations.config import settings
from apps.recommendations.services.keyword_scoring import coerce_keyword_scores, get_top_keywords
...
async def search_papers(
    ...
    *,
    ...
    # Bulk search ignores the ``limit`` query param and can return up to ~1000
    # papers per page. Cap locally so callers never receive more than requested.
    data = payload.get("data")
    if isinstance(data, list) and len(data) > limit:
        logger.info(
            "Semantic Scholar bulk response trimmed query=%r returned=%s limit=%s",
            normalized_query,
            len(data),
            limit,
        )

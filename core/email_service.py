        ...
        try:
            ...
            try:
                async with async_session_factory() as fail_session:
                    ...
                    if db_log:
                        # error_message column is not on TransactionalEmailLog; bump updated_at only.
                        db_log.updated_at = datetime.now(timezone.utc)
...
def build_otp_email_html(
    ...
    greeting = f"Dear {normalized_name}," if normalized_name else "Dear User,"
...
async def send_otp_email(
    ...
    subject = f"Your {title}"
    html_content = build_otp_email_html(otp, otp_purpose, full_name=full_name)

...
async def test_mark_device_verified_updates_installation(mock_db):
    ...
    old_last_active = datetime(2026, 1, 1, tzinfo=timezone.utc)
    installation.last_active_at = old_last_active
    db = mock_db()
    now = datetime(2026, 8, 7, 15, 8, 10, 581000, tzinfo=timezone.utc)
    ...
    with (
        ...
        patch(
            "apps.accounts.services.device_otp_service._now",
            return_value=now,
        ),
    ):
        result = await mark_device_verified(db, uuid.uuid4(), "device-1", now=now)

    assert result is installation
    assert installation.is_device_verified is True
    assert installation.verified_at == now
    assert installation.last_active_at == now
    assert installation.is_active is True
    db.add.assert_called()


@pytest.mark.asyncio
async def test_mark_pending_does_not_demote_other_verified_devices(mock_db):
    """OTP trusts the latest pending device only; older verified installs stay trusted."""
    ...
    pending.device_id = "device-new"
    pending.verified_at = None
    previously_verified = _installation(is_active=False, is_device_verified=True)
    previously_verified.device_id = "device-old"
    db = mock_db()
    user_id = uuid.uuid4()
    now = datetime(2026, 8, 7, 15, 8, 10, 581000, tzinfo=timezone.utc)

    async def _mark(db_arg, uid, device_id, *, now=None):
        assert device_id == "device-new"
        pending.is_device_verified = True
        pending.last_active_at = now
    ...
    with (
        ...
        result = await mark_pending_active_device_verified(db, user_id, now=now)

    assert result is pending
    assert pending.is_device_verified is True
    assert pending.last_active_at == now
    # Other devices are never passed to mark_device_verified / never demoted.
    mark_verified.assert_awaited_once_with(db, user_id, "device-new", now=now)
    assert previously_verified.is_device_verified is True


@pytest.mark.asyncio
async def test_previously_verified_device_skips_otp_after_other_device_verified(mock_db):
    user = _user(email_verified_at=datetime.now(timezone.utc))
    db = mock_db()
    old_device = _installation(is_active=False, is_device_verified=True)
    old_device.device_id = "device-old"

    with patch(
        "apps.accounts.services.device_otp_service.get_user_installation",
        AsyncMock(return_value=old_device),
    ):
        installation, is_new_device, needs_otp = await evaluate_device_otp_requirement(
            db,
            user,
    ...
    assert installation is old_device
    assert is_new_device is False
    assert needs_otp is False
